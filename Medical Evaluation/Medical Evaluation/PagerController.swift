...
public class PagerController: UIViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate, UIScrollViewDelegate {
  ...
  // MARK: - public properties
	...
	public var indicatorColor: UIColor = UIColor(red: 169.00/255.00, green: 0, blue: 0, alpha: 1)
	public var tabsViewBackgroundColor: UIColor = UIColor.whiteColor()// view background color
	...
	private var tabImageNames: [String] = []
	private var tabControllers: [UIViewController] = []
    private var isTapOnTabNeeded : Bool = true
  ...
  // MARK: - Tab and content cache
    ...
    public func setupPager(tabNames tabNames: [String], tabControllers: [UIViewController],isTapOnTabNeeded : Bool = true)
  {
    self.tabImageNames = tabNames
    self.tabControllers = tabControllers
    self.isTapOnTabNeeded = isTapOnTabNeeded
  }
  ...
  //MARK: - Private Methods
	...
	func layoutSubViews() {
		...
		if (self.navigationController?.navigationBar.translucent != false) {
			...
			topLayoutGuide += 44.00 //self.navigationController!.navigationBar.frame.size.height it's hard coded for some crash reason
		}
	...
	func tabViewAtIndex(index: Int) -> TabView? {
		...
		if (self.tabs[index] as UIView?) == nil {
			...
			{
				let title = self.tabImageNames[index]
...
//				tabViewContent = label
                // edited by sreeshaj
                let iconImag = UIImageView(frame: CGRectMake(0, 0, 15, 15))
                iconImag.contentMode = UIViewContentMode.ScaleAspectFit
                iconImag.image = UIImage(named:title)
                tabViewContent = iconImag
