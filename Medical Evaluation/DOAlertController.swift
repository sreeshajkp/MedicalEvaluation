...
class DOAlertController : UIViewController, UITextFieldDelegate, UIViewControllerTransitioningDelegate {
    ...
    var alertViewBgColor = UIColor.MEAlertColor().colorWithAlphaComponent(0.9)   // AlertView Background color
    ...
    var messageTextColor = UIColor.whiteColor()
    ...
    var buttonBgColor: [DOAlertActionStyle : UIColor] = [
        ...
        .Destructive  : UIColor.yellowColor()
    ]
...
extension UIViewController{
    func showAlertController(title:String,message:String,cancelButton:String,isTextFiled:Bool = false,otherButtons buttonTitles:[String],handler: ((Int!) -> Void)!){
    ...
    func showAlertControllerWithTextField(title:String,message:String,cancelButton:String,otherButtons buttonTitles:[String],handler: ((NSMutableDictionary!) -> Void)!){
        
        dispatch_async(dispatch_get_main_queue()) { () -> Void in
            let alertController = DOAlertController(title: title, message: message, preferredStyle: .Alert)
            var commonTextField : UITextField?
                alertController.addTextFieldWithConfigurationHandler({ (textField) in textField.placeholder = kEnterEmailOrUsername;
            ...
            let cancelAction = DOAlertAction(title: cancelButton, style: .Cancel, handler: { (action) -> Void in
                if let _ = handler{
                    ...
                    dictValue.setObject(0, forKey: "index")
                    handler(dictValue)
                }
            })
            alertController.addAction(cancelAction)
                    ...
                    if buttonTitle != ""{
                        let okAction = DOAlertAction(title: buttonTitle, style: .Default, handler: { (action) -> Void in
                            if let buttonIndex = buttonTitles.indexOf(buttonTitle){
                                if let _ = handler{
                                    let dictValue = NSMutableDictionary()
                                    dictValue.setObject(buttonIndex+1, forKey: "index")
                                     dictValue.setObject(commonTextField!, forKey: "textField")
                                   handler(dictValue)
                                }
                            }
                        })
